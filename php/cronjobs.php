...
$currentDate = date('Y-m-d'); //this will get the current date ie when this was posted 2107-07-06
	  ...
	  $remind_query1 = "SELECT * FROM event WHERE reminder= '$currentDate' "; //Sql query to find users that reminders dates match current date.
	...
	if($run1 = $conn->query($remind_query1))
    {
	 $rows = $run1->num_rows;
      ...
      for ($j = 0; $j < $rows; ++$j)//loop through each user in results and send a reminder email.
      {
        ...
        $email_body = "Hi ".$row[3]."\nJust a reminder that am testing you about your event coming up  on the ".$row[7].",\n"
